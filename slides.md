...
- Часть 2: Демо — ~60 мин
  ...
  - Разбор плейбука-аудита (read-only shell)
...
```bash
# setup.sh — повторный запуск даёт неожиданные результаты
apt install -y clickhouse-server        # уже установлен — OK (apt сам по себе идемпотентен)
useradd clickhouse                      # пользователь уже есть — ERROR
mkdir /var/lib/clickhouse-keeper        # каталог уже есть — ERROR
echo "listen 0.0.0.0" >> config.xml     # дубль строки — скрытое повреждение конфига
```
...
- Нет отката: ошибка на шаге 7 из 10 — восстановление вручную
...
- **Ansible** — инструмент автоматизации: описывается *желаемое состояние*, Ansible приводит систему к нему
- Agentless: работает через SSH, на управляемые узлы не требуется устанавливать агентов
...
(Puppet/Chef) — на управляемых узлах не требуется постоянно работающий процесс.

**Два пакета — не путать:**

| Пакет | Что внутри | Когда |
|---|---|---|
| `ansible-core` | движок + модули `ansible.builtin` | предсказуемые версии, коллекции устанавливаются явно |
...
**Что меняет 2.19:** переписанный шаблонизатор (**Data Tagging**) — отслеживает
происхождение и доверие каждого значения; строже к типам и условиям, недоверенные
данные не рендерятся как шаблон. Подробный разбор — на слайде «Data Tagging».
...
```
Control Node (рабочая машина инженера / CI-runner)
│
...
**Ключевая идея:** вся логика выполнения находится на control node — управляемые узлы остаются без дополнительного ПО.

---

## Подготовка окружения

**1. venv — фиксируем версию ansible, не затрагивая системный Python:**
...
```bash
ansible all -i hosts.yaml -m ansible.builtin.ping     # связность
ansible all -i hosts.yaml -m ansible.builtin.setup    # собрать факты о ноде
# если inventory задан в ansible.cfg — флаг -i можно опустить
```
...
```bash
ansible-inventory -i inventories/clickhouse/yc.py --list   # JSON с актуальными IP
```
...
**Ключевая идея:** используйте **FQCN** (Fully Qualified Collection Name) — `ansible.builtin.apt`, а не `apt`. FQCN исключает конфликты имён между коллекциями.
...
## Задача (task): анатомия

**Task** — минимальная единица работы: один вызов модуля с человекочитаемым именем.

```yaml
# Реальная задача из roles/clickhouse/tasks/install.yml
- name: "Установить clickhouse-server и clickhouse-client"   # имя — видно в логе
  ansible.builtin.apt:          # какой модуль вызываем
    name:                       # параметры модуля
      - clickhouse-server
      - clickhouse-client
    state: present              # желаемое состояние
```

- **`name`** — по нему задача читается в логе прогона; имя должно быть осмысленным
- **модуль + параметры** — *что* и в *каком состоянии* должно быть
- **директивы** — поведение задачи: `when`, `loop`, `register`, `notify`,
  `become` — навешиваются рядом с модулем (каждую разберём дальше)

**Статусы** — итог задачи на каждом хосте:

| Статус | Значение |
|---|---|
| `ok` | состояние уже соответствует желаемому — ничего не делали |
| `changed` | модуль внёс изменение |
| `failed` | ошибка — хост выбывает из остатка плея |
| `skipped` | условие `when` не выполнилось |
| `unreachable` | нет SSH-связи с хостом |

**Ключевая идея:** задача = имя + модуль + директивы; статусы `ok`/`changed`/`failed` — язык, которым Ansible отчитывается о каждом шаге (их сводка — `PLAY RECAP`).
...
- `ok` — состояние уже соответствует желаемому, изменения не вносятся
...
**Ключевая идея:** идемпотентный playbook можно запускать многократно — лишних изменений не будет.
...
вызывается как задача и создаёт новую переменную во время выполнения. Такой «факт»
привязан к хосту и живёт до конца **всего прогона playbook** (переживает границы плеев).
...
| | `register` | `set_fact` |
|---|---|---|
| Что сохраняет | весь результат задачи (`stdout`, `rc`, `changed`, …) | ровно то, что задано выражением |
| Кто задаёт значение | модуль — автоматически | пользователь — любым выражением |
| Назначение | сохранить необработанный вывод | извлечь/преобразовать нужное |

Типичная связка: `register` сохраняет необработанный вывод → `set_fact`
извлекает из него нужное значение (в примере: `ch_ping` → `replica_count`).
...
`shell` — не антипаттерн сам по себе: для **read-only снимка** состояния это
уместный инструмент. Парный пример — плейбук-аудит
[erlong15/ansible](https://github.com/erlong15/ansible):

```yaml
# collect_info: ~40 задач, все построены по одной схеме
- name: GET LISTENING PORTS
  ...
  failed_when: false      # ошибка проверки не прерывает прогон
```
...
**Ключевая идея:** выбирайте инструмент по задаче. Снимок — `shell`; изменение состояния — модули. Антипаттерн — это `shell` *вместо* модулей для изменения, а не сам аудит.
...
- Каждая задача выполняется на хостах параллельно — группами по `forks` (по умолчанию 5)
- `forks` задаётся в `ansible.cfg` (`forks = 20`), флагом `-f 20` или через `ANSIBLE_FORKS`

**Ключевая идея:** разделяйте Keeper и Server на разные plays — это даёт контроль над порядком запуска.

---

## Анатомия play: полная структура

```yaml
- name: Настройка ClickHouse Server        # имя плея
  hosts: clickhouse                        # группа / хост / паттерн из инвентори
  become: true                             # sudo для всех задач плея
  serial: 1                                # выполнять хосты группами (rolling)
  gather_facts: true                       # собрать факты (умолчание: true)
  vars:                                    # переменные уровня плея
    clickhouse_component: server

  pre_tasks:                               # задачи ДО ролей (подготовка)
    - name: Проверить связность
      ansible.builtin.ping:

  roles:                                   # роли — статически, после pre_tasks
    - role: clickhouse
  ...
  post_tasks:                              # задачи в самом конце
    - name: Итог
      ansible.builtin.debug: {msg: "Кластер готов"}

  handlers:                                # обработчики — по notify
    - name: Restart clickhouse-server
      ansible.builtin.systemd: {name: clickhouse-server, state: restarted}
```

**Порядок фиксирован:** `pre_tasks` → `roles` → `tasks` → `post_tasks`; handlers
срабатывают в конце каждой фазы. Частая ловушка: `roles:` выполняются **до**
`tasks:`, даже если в файле записаны ниже.

**Ключевая идея:** у play пять фаз с жёстким порядком — pre_tasks, roles, tasks, post_tasks и handlers; расположение секций в файле на порядок не влияет.

---

## Запуск: become, --check, --diff

**`become` — эскалация привилегий** (sudo), без прямого SSH-входа под root:
...
- Срабатывает **один раз**, не сразу после `notify` — а в конце фазы задач
  (для обычного плея это конец play; точный порядок фаз — на слайде «Анатомия play»)
- Сколько бы задач ни уведомили handler — выполнится однократно
...
**Три уровня управления параллельностью:**

| Механизм | Уровень | Что ограничивает |
|---|---|---|
| `forks` (ansible.cfg / `-f`) | глобальный | сколько хостов Ansible обрабатывает одновременно |
| `serial` | play | группа хостов на **весь плей** (rolling deploy) |
| `throttle` | task / handler | **одна конкретная задача** |

Итоговая параллельность задачи = **min(forks, serial, throttle)**.
...
- Если факты не нужны — отключите сбор: `gather_facts: false` (как в нашем `validate.yml`), плей стартует быстрее
...
## Data Tagging: новый шаблонизатор ansible-core 2.19

**Data Tagging** — движок прикрепляет к каждому значению метаданные:
**происхождение** (файл, строка, колонка) и **уровень доверия**.

**Что это даёт:**

1. **Доверие инвертировано.** Раньше любая строка могла отрендериться как шаблон.
   Теперь рендерятся только строки из доверенных источников (playbook, роль,
   vars-файлы). Вывод модулей, факты с нод, внешние данные — **не рендерятся**:
   закрыт целый класс инъекций шаблонов (RCE).
2. **Точные ошибки.** Сообщение показывает файл/строку/колонку, где значение родилось.
3. **Ленивая шаблонизация.** Вложенные структуры рендерятся при обращении — быстрее.

**Что перестанет работать при обновлении (проверьте свои роли):**

```yaml
when: inventory_hostname               # БЫЛО: truthy-строка → true; СТАЛО: ошибка
when: inventory_hostname | length > 0  # правильно: условие возвращает булево
```

- условия (`when` / `until` / `assert`) обязаны возвращать **булево**, не «truthy»
- строже с типами: список, случайно превращённый в строку, теперь виден сразу
- обращение к undefined-переменным падает раньше и последовательнее

**Ключевая идея:** 2.19 знает происхождение и доверие каждого значения — безопаснее и понятнее ошибки; перед обновлением протестируйте роли: truthy-условия в `when` теперь вызывают ошибку.

---

## Обработка ошибок: block / rescue / always

**`block` / `rescue` / `always`** — try/catch/finally для группы задач:

```yaml
- block:
    - ansible.builtin.command: рискованная_операция
  rescue:
    - ansible.builtin.command: ./rollback.sh      # встроенного отката НЕТ — задачи отката пишутся вручную
    - ansible.builtin.fail:                       # критичная ошибка — завершаем явно,
        msg: "деплой не удался, выполнен откат"   #   иначе плей продолжится дальше
  always:
    - ansible.builtin.debug: {msg: "выполнится в любом случае"}
```

**Чего rescue НЕ делает — главные заблуждения:**

- **не откатывает изменения** — транзакций нет; откат — обычные задачи, пишутся вручную
- **не останавливает плейбук**: успешный rescue снимает `failed`, плей идёт дальше
  (не триггерит `any_errors_fatal` / `max_fail_percentage`); критичный сценарий —
  явный `ansible.builtin.fail` или `meta: end_play` внутри rescue
- **не ловит `unreachable`** (обрыв SSH) и ошибки синтаксиса задач — только статус `failed`
- **не сработает при «разрешённых» ошибках**: `ignore_errors: true` или
  `failed_when: false` — задача не failed, перехватывать нечего

Спасённые задачи видны в `PLAY RECAP` счётчиком `rescued=` (в демо он был `0`).

**Ключевая идея:** rescue — try/catch без транзакций: перехватывает только `failed`; откат и остановка при критичной ошибке описываются явно.

---

## Теги: выборочный запуск

**Теги** — метки на задачах для запуска части плейбука:
...
ansible-playbook site.yml --list-tags        # посмотреть все теги плейбука
```

- **Наследование:** тег на `block` или роли применяется к задачам внутри;
  на секции `rescue`/`always` надёжнее ставить теги явно
- **Спец-теги:** `always` — задача выполняется при любом `--tags`
  (кроме `--skip-tags always`); `never` — только если тег запрошен явно

**Ключевая идея:** теги — выборочный запуск части плейбука; учитывайте, что пропущенные задачи могли готовить состояние для оставшихся.

---

## Циклы: loop

**`loop`** повторяет одну задачу для каждого элемента списка — вместо дублирования задач.
...
**Ключевая идея:** `loop` устраняет дублирование задач; `item` — текущий элемент. Пароль-шаблон в нашей роли намеренно вне цикла — у него `no_log: true`, иначе вывод скрылся бы для всех итераций. Старый синтаксис `with_items` официально не запрещён, но в новом коде используйте `loop`.
...
**Ключевая идея:** `until/retries/delay` — цикл «повторять до выполнения условия»; основа health-gate и защиты от гонок (репликация, старт сервиса).
...
└── meta/
    ├── main.yml            # galaxy_info + dependencies (роли-зависимости)
    └── argument_specs.yml  # валидация входных переменных
```

- Одна роль — один компонент; повторное использование — через передачу переменных
...
Не обязательно писать всё самостоятельно — Ansible Galaxy предоставляет готовые
роли (`geerlingguy.postgresql` и др.). Зависимости фиксируются в **`requirements.yml`**:
...
- `include_role` / `import_role` из задач — динамически или статически (разбор на следующем слайде)
- переопределение переменных — через `vars:` при подключении (значения по умолчанию у роли — самые слабые)
- зависимости роли — в `meta/main.yml` → `dependencies:` (роль подключает другие роли)

**Наш выбор:** собственная роль — контроль, обучение и независимость от внешних
зависимостей (доступ к Galaxy из РФ нестабилен).
...
## include vs import: динамика и статика

Два способа подключить роль или файл задач — разница в **моменте обработки**:

| | `import_role` / `import_tasks` | `include_role` / `include_tasks` |
|---|---|---|
| Когда обрабатывается | при **парсинге** плейбука | во время **выполнения** |
| Аналогия | «вставить текст как есть» | вызов функции в рантайме |
| Имя из переменной | нельзя | можно: `name: "{{ component }}"` |
| В цикле `loop` | нельзя | можно |
| `when` на директиве | копируется на **каждую** задачу внутри | проверяется **один раз** при подключении |
| Теги и `--list-tasks` | видят задачи насквозь | вложенные задачи не видны (тегам нужен `apply:`) |

```yaml
# Реальный пример из roles/clickhouse/tasks/main.yml — динамический диспетчер:
- name: "Настроить компонент keeper"
  ansible.builtin.include_tasks: keeper.yml    # решение принимается в рантайме
  when: clickhouse_component == 'keeper'
```

**Практическое правило:** нужна условная логика, цикл или имя из переменной →
`include_*`; нужны сквозные теги, `--list-tasks` и предсказуемость → `import_*`.

**Ключевая идея:** import — статика на этапе парсинга, include — динамика в рантайме; наш `main.yml` использует `include_tasks` именно ради `when`-диспетчера компонентов.

---

## argument_specs.yml — самодокументирующийся интерфейс роли

**Проблема:** без валидации роль принимает некорректные входные данные и завершается ошибкой в непредсказуемом месте.
...
- `defaults/main.yml` — безопасные значения по умолчанию роли, поэтому у них низший приоритет
- `host_vars` сильнее `group_vars` — точечное переопределение для одной ноды
- `-e key=value` — разовое переопределение, приоритетнее любого файла

**Ключевая идея:** `defaults` — слабейшие, `-e` — сильнейшие; знание приоритета экономит часы отладки при поиске источника значения переменной.
...
**высший приоритет**: переопределяют значение из любого другого источника.
...
# из файла — по файлу на контур развёртывания (dev/stage/prod)
ansible-playbook site.yml -e @environments/prod.yml
```

- Когда применять: разовое переопределение, параметры из CI-пайплайна,
  переключение **контура развёртывания** (что это — следующий слайд)
- Внимание: поскольку `-e` переопределяет всё, легко непреднамеренно перекрыть
  значение из `group_vars`/`host_vars`
...
## Контур развёртывания: dev / stage / prod

**Контур** — изолированный комплект «инфраструктура + параметры» под один и тот же
код: dev — для экспериментов, stage — для проверки, prod — производственный. В Ansible нет
встроенного понятия «контур» — он задаётся **структурой проекта**.

**Способ 1 — инвентори на контур (канонический):** различаются и хосты, и параметры.

```
inventories/
├── dev/
│   ├── hosts.yaml           # одна маленькая ВМ
│   └── group_vars/...       # dev-параметры, dev-секреты
└── prod/
    ├── hosts.yaml           # 3 ноды
    └── group_vars/...       # prod-параметры, prod-секреты
```

```bash
ansible-playbook -i inventories/prod playbooks/clickhouse/site.yml
```

**Способ 2 — vars-файл на контур:** хосты общие, различаются только параметры.

```bash
ansible-playbook site.yml -e @environments/prod.yml
```

- Наш проект — один контур (учебный стенд): `inventories/clickhouse/`.
  Второй контур — это просто ещё один каталог рядом
- Не путать с переменными окружения ОС — про них следующий слайд

**Ключевая идея:** код один — контуров много; контур выбирается в момент запуска: свой инвентори (`-i`) или свой vars-файл (`-e @`).
...
**Ключевая идея:** используйте `ansible.builtin.deb822_repository` для новых репозиториев — структурированный формат `*.sources`, читаемый и легко управляемый Ansible.
...
**Когда НЕ Ansible:**
- Stateful оркестрация контейнеров — задача Kubernetes
- K8s-объекты (Deployment, Service) — Helm / kubectl
- Создание инфраструктуры (VPC, VM) — Terraform / OpenTofu
