...
- **`name`** — по нему задача читается в логе прогона; всегда пиши осмысленно
...
- name: Настройка ClickHouse Server        # имя плея
  ...
  serial: 1                                # выполнять хосты пачками (rolling)
...
| `serial` | play | пачка хостов на **весь плей** (rolling deploy) |
...
**Что сломается при апгрейде (проверь свои роли):**
...
**Ключевая идея:** 2.19 знает происхождение и доверие каждого значения — безопаснее и понятнее ошибки; перед апгрейдом прогони роли: truthy-условия в `when` теперь падают.
...
- block:
    ...
    - ansible.builtin.command: ./rollback.sh      # отката «из коробки» НЕТ — пишем сами
    - ansible.builtin.fail:                       # критичная ошибка? — валим явно,
        msg: "деплой не удался, откатились"       #   иначе плей продолжится дальше
  always:
...
- **не откатывает изменения** — транзакций нет; откат — обычные задачи, пишешь сам
...
**Ключевая идея:** rescue — try/catch без транзакций: ловит только `failed`; откат и остановку критичной ошибки описываешь явно.
...
**Ключевая идея:** теги — выборочный запуск части плейбука; помни, что пропущенные задачи могли готовить состояние для оставшихся.
